...
public interface UserClient {
    ...
    // Fallback method
    default ResponseEntity<ApiResponse<UserResponseDto>> getUserByIdFallback(Long userId, Exception ex) {
        // 사용자 정보 조회 실패 시
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.error("Failed to fetch user information", null));
    }
}